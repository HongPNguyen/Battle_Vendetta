...
namespace IV.BaseUnity
{
	public class SetPivotWindow : EditorWindow
	{
		...
		void UtilitiesGUI()
		{
			...
			{
				_x = _y = _z = .5f;
				_pivot = _mesh ? _transform.TransformPoint(_mesh.bounds.center) : GetEmptyBounds().center;
				UpdatePivot();
			}
		...
		Bounds GetEmptyBounds()
		{
			Bounds[] bounds = _transform.GetComponentsInChildren<MeshRenderer>()
				.Select(r => r.bounds)
				.ToArray();
